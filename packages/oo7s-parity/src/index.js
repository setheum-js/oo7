...
class Bonds {
	/**
	 * Creates a new oo7s-parity bonds aggregate object with given ethereum provider.
	 *
	 * Additional documentation can be found at https://wiki.parity.io/oo7s-Parity-Reference.html
	 *
	...
	constructor (provider = defaultProvider()) {
		 ...
		 *
		 * @example
		 * const { bonds } = require('oo7s-parity')
		 *
		 * bonds
		 *	.time
...
class DeployContract extends oo7s.ReactivePromise {
...
class Transaction extends oo7s.ReactivePromise {
	constructor (tx, api) {
		super([tx], [], ([tx]) => {
			let progress = this.trigger.bind(this);
...
function createBonds (options) {
	...
	class TransformBond extends oo7s.TransformBond {
	...
	class SubscriptionBond extends oo7s.Bond {
	...
	class Signature extends oo7s.ReactivePromise {
	...
	function presub (f) {
		return new Proxy(f, {
			get (receiver, name) {
				...
				} else if (typeof (name) === 'symbol' && oo7s.Bond.knowSymbol(name)) {
					return receiver(oo7s.Bond.fromSymbol(name));
				} else {
	...
	bonds.registry.names = oo7s.Bond.mapAll([bonds.registry.ReverseConfirmed({}, { limit: 100 }), bonds.accountsInfo],
		(reg, info) => {
			let r = {};
...
const isOwned = addr => oo7s.Bond.mapAll([addr, bonds.accounts], (a, as) => as.indexOf(a) !== -1);
const isNotOwned = addr => oo7s.Bond.mapAll([addr, bonds.accounts], (a, as) => as.indexOf(a) === -1);
